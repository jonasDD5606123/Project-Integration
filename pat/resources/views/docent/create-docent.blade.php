...
<body>
    ...
    <div class="card-glass mt-4">
        <h2 class="text-center mb-4">Docent toevoegen</h2>
        ...
        <form method="POST" action="{{ route('create-docent') }}">
            ...
            <div class="mb-3">
                ...
                <input id="email" class="form-control" type="email" name="email" value="{{ old('email') }}" placeholder="voorbeeld@docent.be" required />
                @error('email')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <button type="submit" class="btn btn-custom w-100">Docent toevoegen</button>
        </form>
    </div>
